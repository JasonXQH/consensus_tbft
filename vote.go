...
func (vs *VoteSet) checkVoteMatched(vote *tbftpb.Vote) bool {
	//
	if (vote.Type == tbftpb.VoteType_VOTE_MODIFYPREVOTE && vs.Type == tbftpb.VoteType_VOTE_PREVOTE) ||
		(vote.Type == tbftpb.VoteType_VOTE_MODIFYPRECOMMIT && vs.Type == tbftpb.VoteType_VOTE_PRECOMMIT){
		return true
	}
...
func (vs *VoteSet) AddVote(vote *tbftpb.Vote, countInvalidTx bool) (added bool, err error) {
	...
	if !vs.validators.HasValidator(vote.Voter) {
		vs.logger.Infof("xqh invalide validator!!")
		return false, fmt.Errorf("%w %s", ErrInvalidValidator, vote.Voter)
	}
	//如果该投票者已有投票记录，检查新旧投票的哈希是否一致。如果不一致，表示同一投票者对同一事项有冲突投票，返回错误
	...
	//将投票添加到 vs.Votes 映射中，增加总投票数 vs.Sum。
	...
	vs.logger.Infof("VoteSet(%s/%d/%d) AddVote %s(%s/%d/%d/%x) "+
	...
	//如果达到了法定人数，并且之前没有记录达到法定人数的哈希，则更新 vs.Maj23 为当前哈希，并将所有相关的投票记录到主投票集。
...
func (hvs *heightRoundVoteSet) getVoteSet(round int32, voteType tbftpb.VoteType) *VoteSet {
	...
	switch voteType {
	case tbftpb.VoteType_VOTE_PREVOTE, tbftpb.VoteType_VOTE_MODIFYPREVOTE:
		return rvs.Prevotes
	case tbftpb.VoteType_VOTE_PRECOMMIT, tbftpb.VoteType_VOTE_MODIFYPRECOMMIT:
...
func (hvs *heightRoundVoteSet) addVote(vote *tbftpb.Vote) (added bool, err error) {
	...
	if voteSet == nil {
		hvs.logger.Infof("xqh voteSet = nil 新建voteSet")
...
func createPrevoteModifyMsg(prevote *tbftpb.Vote) *ConsensusMsg {
	return &ConsensusMsg{
...
func createPrecommitModifyMsg(precommit *tbftpb.Vote) *ConsensusMsg {
	return &ConsensusMsg{
...
type ConsensusDatabase struct {
	ProposalsByConsensusID map[string][]*tbftpb.Proposal        // Maps consensus ID to a slice of Proposals
	VoteSetsByHeightRound  map[uint64]map[int32]*roundVoteSet // Nested map: height -> round -> VoteSet
}

func NewConsensusDatabase() *ConsensusDatabase {
	return &ConsensusDatabase{
		ProposalsByConsensusID: make(map[string][]*tbftpb.Proposal  ),
		VoteSetsByHeightRound:  make(map[uint64]map[int32]*roundVoteSet),
	}
}

func (db *ConsensusDatabase) AddProposal(consensusID string, proposal *tbftpb.Proposal   ) {
	db.ProposalsByConsensusID[consensusID] = append(db.ProposalsByConsensusID[consensusID], proposal)
}

func (db *ConsensusDatabase) GetProposals(consensusID string) []*tbftpb.Proposal   {
	return db.ProposalsByConsensusID[consensusID]
}

func (db *ConsensusDatabase) GetOrCreateVoteSet(height uint64, round int32) *VoteSet {
	if db.VoteSetsByHeightRound[height] == nil {
		db.VoteSetsByHeightRound[height] = make(map[int32]*VoteSet)
	}

	if _, exists := db.VoteSetsByHeightRound[height][round]; !exists {
		db.VoteSetsByHeightRound[height][round] = &heightRoundVoteSet{
			Height:        height,
			Round:         round,
			RoundVoteSets: make(map[int32]*RoundVoteSet),
		}
	...
	return db.VoteSetsByHeightRound[height][round]
}
