...
//订阅注册信息
var msgBusTopics = []msgbus.Topic{msgbus.ModifyBlock, msgbus.RecvModifyBlock ,msgbus.ProposedBlock, msgbus.VerifyResult,
...
type ConsensusTBFTImpl struct {
	...
	*ModifyRequestState
	...
	modifyStateCache *modifyStateCache
	...
	//modifyFutureMsgCache *ModifyFutureMsgCache
	...
	//
	modifyRequestBlockC chan *common.Block
...
func New(config *consensusUtils.ConsensusImplConfig) (*ConsensusTBFTImpl, error) {
	...
	consensus.modifyRequestBlockC = make(chan *common.Block, defaultChanCap)
	...
	//xqh add
	consensus.ModifyRequestState = NewModifyRequestState(consensus.logger, consensus.Id)
	consensus.modifyStateCache = newModifyStateCache(defaultConsensusStateCacheSize)

	consensus.consensusFutureMsgCache = newConsensusFutureMsgCache(consensus.logger,
		defaultConsensusFutureCacheSize, 0)
	//consensus.modifyFutureMsgCache = newModifyFutureMsgCache()
...
/*
如果存在同步服务且验证集大小不为1，它将等待同步服务达到理想高度。
为各种消息主题注册消息总线。
启动时间调度器 timeScheduler。
初始化一致性引擎。
重放写入前日志（WAL）。
启动一致性引擎。
启动处理消息的协程。
...
/*
 ...
 方法用于通过消息总线发送提议状态。它记录日志并发布提议状态消息。
*/
...
func (consensus *ConsensusTBFTImpl) OnMessage(message *msgbus.Message) {
	...
	case msgbus.ModifyBlock:
		if modifyRequestBlock, ok := message.Payload.(*common.Block); ok {
			if modifyRequestBlock.Header.BlockHeight == 0  {
				consensus.logger.Errorf("receive message failed, error message modify blockheight = 0")
				return
			}
			consensus.logger.Infof("xqh收到来自上层的修改区块请求，准备共识！")
			currentTime := time.Now()
			fmt.Println("开始当前时间是:", currentTime, "Block Height:", modifyRequestBlock.Header.BlockHeight)
			consensus.enterNewModifyHeight(modifyRequestBlock.Header.BlockHeight)
			consensus.modifyRequestBlockC <- modifyRequestBlock
		} else {
			consensus.logger.Warnf("assert modify block failed, get type:{%s}", reflect.TypeOf(message.Payload))
		}
	case msgbus.RecvModifyBlock:
		if msg, ok := message.Payload.(*netpb.NetMsg); ok {
			modifyRequestMsg := consensus.createConsensusMsgFromTBFTMsgBz(msg.Payload)
			if  modifyRequestMsg != nil {
				consensus.logger.Infof("xqh 成功收到来自其他节点的RecvModifyBlock信息,准备加入缓存!")
				consensus.externalMsgC <- modifyRequestMsg
			} else {
				consensus.logger.Errorf("xqh assert Modify Msg failed")
			}
		} else {
			consensus.logger.Infof("assert NetMsg failed, get type:{%s}", reflect.TypeOf(message.Payload))
		}
	}
...
func (consensus *ConsensusTBFTImpl) handle() {
	...
	for loop {
		...
		case modifyRequestBlock := <-consensus.modifyRequestBlockC:
			consensus.logger.Infof("[%s] receive from  modifyRequestBlockC", modifyRequestBlock.Header.BlockHeight)
			consensus.handleModifyRequestBlock(modifyRequestBlock)
		case result := <-consensus.verifyResultC:
			if result.Code == consensuspb.VerifyModifyResult_SUCCESS || result.Code == consensuspb.VerifyModifyResult_FAIL{
				consensus.logger.Infof("xqh receive VerifyModifyResult_SUCCESS from  verifier")
				consensus.handleVerifyModifyResult(result)
			}else{
...
func (consensus *ConsensusTBFTImpl) handleConsensusMsg(msg *ConsensusMsg) {
	...
	case tbftpb.TBFTMsgType_MSG_MODIFY_PREVOTE:
		consensus.logger.Infof("xqh来自enterPrevote(internalC)或者其他节点发送的预投票信息(externalC)")
		consensus.procModifyPrevote(msg.Msg.(*tbftpb.Vote))
	case tbftpb.TBFTMsgType_MSG_MODIFY_PROPOSE:
		consensus.procModifyPropose(msg.Msg.(*tbftpb.Proposal))
	case tbftpb.TBFTMsgType_MSG_MODIFY_PRECOMMIT:
		consensus.logger.Infof("xqh来自enterPrecommit(internalC)或者其他节点发送的预提交信息(externalC)")
		consensus.procModifyPrecommit(msg.Msg.(*tbftpb.Vote))
	}
}
...
func (consensus *ConsensusTBFTImpl) addPrevoteVote(vote *tbftpb.Vote) {
	...
	if !ok || voteSet.Round != consensus.Round {
		consensus.logger.Infof("[%s](%d/%d/%s) addVote %v without majority",
		...
		if consensus.Round == vote.Round && voteSet.hasTwoThirdsAny() {
			consensus.logger.Infof("[%s](%d/%d/%s) addVote %v with hasTwoThirdsAny",
				consensus.Id, consensus.Height, consensus.Round, consensus.Step, vote)
			consensus.logger.Infof("xqh 准备进入共识enterPrecommit")
	...
	if consensus.Proposal != nil {
		...
		if !isNilHash(hash) && (consensus.ValidRound < vote.Round) && (vote.Round == consensus.Round) {
			if bytes.Equal(hash, consensus.Proposal.PbMsg.Block.Header.BlockHash) {
				consensus.logger.Infof("updating valid block because of maj32, valid_round: %d, majority_round: %d",
...
// enterNewHeight enter `height`
func (consensus *ConsensusTBFTImpl) enterNewModifyHeight(height uint64) {
	consensus.logger.Infof("xqh [%s] 进入新高度， (%d)", consensus.Id, height)
	//if consensus.Height >= height {
	//	consensus.logger.Errorf("[%s](%v/%v/%v) invalid enter invalid new height to (%v)",
	//		consensus.Id, consensus.Height, consensus.Round, consensus.Step, height)
	//	return
	...
	consensus.consensusFutureMsgCache.updateConsensusHeight(consensus.Height)
	consensus.modifyStateCache.addModifyRequestState(consensus.ModifyRequestState)
	consensus.ModifyRequestState = NewModifyRequestState(consensus.logger, consensus.Id)
	consensus.ModifyBlockHeight = height
	//consensus.validatorSet.Lock()
	//consensus.validatorSet.validatorsHeight[consensus.Id] = height
	//consensus.validatorSet.Unlock()
	consensus.ModifyBlockRound = 0
	//consensus.LockedRound = -1
	//consensus.LockedProposal = nil
	//consensus.ValidRound = -1
	//consensus.ValidProposal = nil
	//consensus.ModifyConsensusStep = tbftpb.Step_NEW_HEIGHT
	consensus.modifyHeightRoundVoteSet = newHeightRoundVoteSet(
		consensus.logger, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.validatorSet)
	//consensus.metrics = newHeightMetrics(consensus.Height)
	//consensus.metrics.SetEnterNewHeightTime()
	//consensus.enterNewRound(height, 0)
}
...
func (consensus *ConsensusTBFTImpl) enterNewRound(height uint64, round int32) {
	...
	if consensus.blockVersion >= blockVersion231 {
		...
		//consensus.logger.Infof("xqh consensus.blockVersion >= blockVersion231 成立")
...
func (consensus *ConsensusTBFTImpl) createConsensusMsgFromTBFTMsgBz(tbftMsgBz []byte) *ConsensusMsg {
	...
	case tbftpb.TBFTMsgType_MSG_MODIFY_PROPOSE:
		proposal := new(tbftpb.Proposal)
		mustUnmarshal(tbftMsg.Msg, proposal)
		consensus.logger.Infof("xqh 收到来自其他节点的Modify Propose信息")
		return &ConsensusMsg{
			Type: tbftMsg.Type,
			Msg:  proposal,
		}
	case tbftpb.TBFTMsgType_MSG_MODIFY_PREVOTE:
		prevote := new(tbftpb.Vote)
		mustUnmarshal(tbftMsg.Msg, prevote)
		consensus.logger.Infof("xqh 收到来自其他节点的 MODIFY_PREVOTE 信息, %s(%d/%d/%x)",prevote.Voter, prevote.Height, prevote.Round, prevote.Hash)
		if err := consensus.verifyVote(prevote); err != nil {
			consensus.logger.Errorf("xqh [%s](%d/%d/%s) receive modify prevote %s(%d/%d/%x), verifyVote failed: %v",
				consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,
				prevote.Voter, prevote.Height, prevote.Round, prevote.Hash, err,
			)
			return nil
		}else {
			consensus.logger.Infof("xqh 验证 MODIFY_PREVOTE 通过")
		}
		return &ConsensusMsg{
			Type: tbftMsg.Type,
			Msg:  prevote,
	...
	case tbftpb.TBFTMsgType_MSG_MODIFY_PRECOMMIT:
		precommit := new(tbftpb.Vote)
		mustUnmarshal(tbftMsg.Msg, precommit)
		consensus.logger.Infof("xqh 收到来自其他节点的 MODIFY_PRECOMMIT 信息  %s(%d/%d/%x)",precommit.Voter, precommit.Height, precommit.Round, precommit.Hash)
...
// @param proposedBlock
//
func (consensus *ConsensusTBFTImpl) handleModifyRequestBlock(modifyRequestBlock *common.Block) {
	//return
	consensus.Lock()
	defer consensus.Unlock()
	//
	modifyRequestBlock.Header.TxRoot =[]byte(consensus.Id)
	//// consensus not using the same block pointer with core, but internal content using the same pointer
	//block := CopyBlock(proposedProposal.proposedBlock.Block)
	consensus.logger.Infof("receive Modify Header from core engine (%d/%x), requester: %s",
		modifyRequestBlock.Header.BlockHeight, modifyRequestBlock.Header.BlockHash, consensus.Id,
	)
	consensus.ModifyBlockHeight = modifyRequestBlock.Header.BlockHeight

	//// Add proposal
	modifyProposal := NewProposal(consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, -1, modifyRequestBlock)
	//// this is not a locked proposal

	sig, _, err := utils.SignBlock(consensus.chainConf.ChainConfig().Crypto.Hash, consensus.signer, modifyRequestBlock)
	if err != nil {
		consensus.logger.Errorf("[%s]sign modify block Header failed, %s", consensus.Id, err)
		return
	}
	modifyRequestBlock.Header.Signature = sig

	err = consensus.signProposal(modifyProposal)
	if err != nil {
		consensus.logger.Errorf("sign proposal err %s", err)
		return
	}
	modifyProposal.Block = modifyRequestBlock
	consensus.ModifyProposal = NewTBFTProposal(modifyProposal, true)
	consensus.logger.Infof("xqh 构造新NewTBFTProposal 成功")
	consensus.sendModifyProposal(consensus.ModifyProposal, "")
	consensus.logger.Infof("xqh 发送 ModifyProposal 成功")
	//// Enter the prevote stage
	consensus.enterModifyPrevote(consensus.ModifyBlockHeight, consensus.ModifyBlockRound)
}
// enterPrevote enter `prevote` phase
func (consensus *ConsensusTBFTImpl) enterModifyPrevote(height uint64, round int32) {
	//if consensus.Height != height ||
	//	consensus.Round > round ||
	//	(consensus.Round == round && consensus.Step != tbftpb.Step_PROPOSE) {
	//	consensus.logger.Infof("xqh enterModifyPrevote enter invalid ")
	//	return
	//}

	// Enter StepPrevote
	consensus.ModifyConsensusStep = tbftpb.Step_PREVOTE
	// Disable propose
	//consensus.sendProposeState(false)
	...
	// Broadcast prevote
	// prevote := createPrevoteProto(consensus.Id, consensus.Height, consensus.Round, hash)
	prevote := NewVote(tbftpb.VoteType_VOTE_MODIFYPREVOTE, consensus.Id,height,round, hash)
	...
	err := consensus.signVote(prevote)
	if err != nil {
		consensus.logger.Errorf("xqh enter enterModifyPrevote sign Vote error: %s", err)
	}
	modifyPrevoteMsg := createPrevoteModifyMsg(prevote)

	consensus.internalMsgC <- modifyPrevoteMsg

	consensus.logger.Infof("[%s](%v/%v/%v)xqh 生成了  Modify Prevote,交给internalMsgC",
		consensus.Id, height, round, consensus.ModifyConsensusStep)
}


func (consensus *ConsensusTBFTImpl) procModifyPrevote(prevote *tbftpb.Vote) {
	// update node height
	// add future prevote in cache
	if consensus.ModifyBlockHeight != prevote.Height {
		consensus.logger.Infof("[%s](%d/%d/%s) receive future prevote %s(%d/%d/%x)",
			consensus.Id, consensus.ModifyBlockHeight,consensus.ModifyBlockHeight, consensus.ModifyBlockHeight,
			prevote.Voter, prevote.Height, prevote.Round, prevote.Hash)
		consensus.consensusFutureMsgCache.addFutureVote(consensus.validatorSet, prevote)
		return
	}
	...
	consensus.logger.Infof("[%s](%d/%d/%s) xqh receive modify prevote %s(%d/%d/%x)",
		consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,
		prevote.Voter, prevote.Height, prevote.Round, prevote.Hash,
	)
	consensus.logger.Infof("xqh 将prevote加入到自己的投票集合中[%s](%d/%d/%s) ",consensus.Id, consensus.ModifyBlockHeight,
		consensus.ModifyBlockRound, consensus.ModifyConsensusStep)

	err := consensus.addModifyVote(prevote, false)
	if err != nil {
		consensus.logger.Errorf("xqh [%s](%d/%d/%s) addVote %s(%d/%d/%s) failed, %v",
			consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,
			prevote.Voter, prevote.Height, prevote.Round, prevote.Type, err,
		)
		return
...
// addVote adds `vote` to heightVoteSet
func (consensus *ConsensusTBFTImpl) addModifyVote(vote *tbftpb.Vote, replayMode bool) error {
	//time.Sleep(100 * time.Millisecond)
	if(consensus.Id == vote.Voter){
		consensus.logger.Infof("收到自己生成的vote，投票信息为:[%s](%d/%d/%s)",vote.Voter,vote.Height,vote.Round,vote.Type)
	}else {
		consensus.logger.Infof("收到其他节点发送的vote ，投票信息为:[%s](%d/%d/%s)",vote.Voter,vote.Height,vote.Round,vote.Type)
	}
	_, err := consensus.modifyHeightRoundVoteSet.addVote(vote)
	if err != nil {
		consensus.logger.Infof(" xqh [%s](%d/%d/%s) addVote %v, err: %v",
			consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep, vote, err)
		return err
	}
	consensus.logger.Infof(" xqh 成功添加vote到自己的modifyHeightRoundVoteSet")
	switch vote.Type {
	case tbftpb.VoteType_VOTE_MODIFYPREVOTE:
		consensus.addModifyPrevoteVote(vote)
	case tbftpb.VoteType_VOTE_MODIFYPRECOMMIT:
		consensus.addModifyPrecommitVote(vote)
	}
	//如果自己是投票者
	if consensus.Id == vote.Voter {
		consensus.logger.Infof(" xqh准备将信息发送给其他节点!!!,%s",vote.Type)
		// save prevote qc when send self precommit vote
		//if vote.Type == tbftpb.VoteType_VOTE_MODIFYPRECOMMIT && !replayMode {
		//	if consensus.ModifyProposal != nil && consensus.ModifyProposal.PbMsg != nil &&
		//		consensus.ModifyProposal.PbMsg.Block != nil &&
		//		bytes.Equal(consensus.ModifyProposal.PbMsg.Block.Header.BlockHash, vote.Hash) {
				// get prevote qc
				//voteSet := consensus.heightRoundVoteSet.prevotes(consensus.Round)
				//hash, ok := voteSet.twoThirdsMajority()
				//walEntry := consensus.Proposal.PbMsg
	...
	//广播给其他节点
		...
		consensus.sendModifyConsensusVote(vote, "")
	}
	return nil
}


func (consensus *ConsensusTBFTImpl) procModifyPropose(proposal *tbftpb.Proposal) {
	//receiveProposalTime := CurrentTime()
	if proposal == nil || proposal.Block == nil {
		consensus.logger.Infof("xqh [%s](%d/%d/%s) receive invalid modify proposal because nil proposal",
			consensus.Id, consensus.Height, consensus.Round, consensus.Step)
		return
	}
	//consensus.ModifyBlockHeight = proposal.Height
	//consensus.ModifyBlockRound = proposal.Round
	//height := proposal.Block.Header.BlockHeight
	//if consensus.ModifyBlockHeight != height || consensus.ModifyBlockRound != proposal.Round || consensus.Step != tbftpb.Step_PROPOSE {
	//	consensus.logger.Debugf("[%s](%d/%d/%s) receive invalid proposal: %s(%d/%d)",
	//		consensus.Id, consensus.Height, consensus.Round, consensus.Step,
	//		proposal.Voter, proposal.Height, proposal.Round)
	//	return
	...
	//}
	//procProposalTime := CurrentTime()
	consensus.enterNewModifyHeight(proposal.Block.Header.BlockHeight)

	consensus.VerifingModifyProposal = NewTBFTProposal(proposal, false)

	// Tell the consensus module to perform block validation
	consensus.msgbus.PublishSafe(msgbus.VerifyModifyBlock, proposal.Block)
	consensus.logger.Infof("xqh 收到其他节点发来的Modify Proposal，并向verifier发送验证修改区块的proposal [%s](%d/%d/%s)",consensus.Id, proposal.Block.Header.BlockHeight)
	//consensus.logger.Infof("[%s](%d/%d/%s) processed proposal (%d/%d/%x), time[verify:%dms]",
	//	consensus.Id, consensus.Height, consensus.Round, consensus.Step,
	//	proposal.Height, proposal.Round, proposal.Block.Header.BlockHash,
	//	procProposalTime.Sub(receiveProposalTime).Milliseconds())
...
func (consensus *ConsensusTBFTImpl) procModifyPrecommit(precommit *tbftpb.Vote) {
	// update node height
	//consensus.updateValidatorHeight(precommit)
	// add future precommit in cache
	//if consensus.ModifyBlockHeight < precommit.Height {
	//	consensus.logger.Infof("[%s](%d/%d/%s) receive future precommit %s(%d/%d/%x)",
	//		consensus.Id, consensus.Height, consensus.Round, consensus.Step,
	//		precommit.Voter, precommit.Height, precommit.Round, precommit.Hash)
	//	consensus.consensusFutureMsgCache.addFutureVote(consensus.validatorSet, precommit)
	...
	// reject the vote that below consensus current height or locked round or current round or step
	if consensus.ModifyBlockHeight != precommit.Height ||
		consensus.LockedModifyRound > precommit.Round ||
		consensus.ModifyBlockRound > precommit.Round ||
		(consensus.ModifyBlockRound == precommit.Round && consensus.ModifyConsensusStep > tbftpb.Step_PRECOMMIT) {
		//consensus.procLocalVote(precommit)
		consensus.logger.Infof("xqh [%s](%d/%d/%s) receive  invalid modify precommit %s(%d/%d)",
			consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,
			precommit.Voter, precommit.Height, precommit.Round,
		)
		return
	...
	// we allow locally generated precommitVote to be added repeatedly
	// because the local node will not be a byzantine node
	if !consensus.modifyHeightRoundVoteSet.isRequired(precommit.Round, precommit) &&
		precommit.Voter != consensus.Id {
		consensus.logger.Infof("xqh receive modify precommit, but the vote is not required")
		return
	}
	...
	consensus.logger.Infof("xqh [%s](%d/%d/%s) receive modify precommit %s(%d/%d/%x), time[verify:%dms]",
		consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,
		precommit.Voter, precommit.Height, precommit.Round, precommit.Hash,
		verifyPrecommitTime.Sub(receivePrecommitTime).Milliseconds())

	err := consensus.addModifyVote(precommit, false)
	if err != nil {
		consensus.logger.Errorf("xqh [%s](%d/%d/%s) add Modify precommit Vote %s(%d/%d/%s) failed, %v",
			consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.Step,
			precommit.Voter, precommit.Height, precommit.Round, precommit.Type, err,
		)
		return
	}
}
func (consensus *ConsensusTBFTImpl) handleVerifyModifyResult(verifyResult *consensuspb.VerifyResult) {
	consensus.Lock()
	defer consensus.Unlock()
	//receiveResTime := CurrentTime()
	//
	...
	////	height, hash, verifyResult.Code)
	//
	//if consensus.VerifingModifyProposal == nil{
	//	consensus.logger.Infof ("xqh [%s](%d/%d/%s) receive verify modify result failed, (%d/%x) %v",
	//		consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,
	//		height, hash, verifyResult.Code,
	//	)
	//	return
	...
	//if consensus.ModifyConsensusStep != tbftpb.Step_PROPOSE ||
	//	!bytes.Equal(consensus.VerifingModifyProposal.PbMsg.Block.Header.BlockHash, hash) {
	//	consensus.logger.Errorf(" xqh  receive verify modify block result (%d/%x) error",
	//		consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,
	//		consensus.VerifingProposal.PbMsg.Block.Header.BlockHash,
	//		height, hash,
	//	)
	...
	consensus.ModifyProposal = consensus.VerifingModifyProposal
	consensus.ModifyProposal.PbMsg.Block = verifyResult.VerifiedBlock
	consensus.logger.Infof("xqh设置成功，进入enterModifyPrevote函数")
	//consensus.ModifyProposal.PbMsg.TxsRwSet = verifyResult.TxsRwSet
	//endTime := CurrentTime()
	//
	//consensus.logger.Infof("[%s](%d/%d/%s)xqh  processed verify modify result (%d/%x), time[verify:%dms]",
	//	consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep, height, hash,
	//	endTime.Sub(receiveResTime).Milliseconds())

	if  verifyResult.Code != consensuspb.VerifyModifyResult_FAIL {
		consensus.logger.Infof("xqh 收到来自验证层验证成功的Modify Proposal ,进入enterModifyPrevote函数")
		consensus.enterModifyPrevote(consensus.ModifyBlockHeight, consensus.ModifyBlockRound)
	}
}
func (consensus *ConsensusTBFTImpl) addModifyPrevoteVote(vote *tbftpb.Vote) {
	if consensus.ModifyConsensusStep != tbftpb.Step_PREVOTE {
		consensus.logger.Infof("xqh [%s](%d/%d/%s) addVote prevote %s(%d/%d/%x) at inappropriate step!!!",
			consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,
			vote.Voter, vote.Height, vote.Round, vote.Hash)
		return
	}
	voteSet := consensus.modifyHeightRoundVoteSet.prevotes(vote.Round)
	hash, ok := voteSet.twoThirdsMajority()
	if !ok || voteSet.Round != consensus.ModifyBlockRound {
		consensus.logger.Infof("xqh [%s](%d/%d/%s) addVote %v without majority!!",
			consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep, vote)

		if consensus.ModifyBlockRound == vote.Round && voteSet.hasTwoThirdsAny() {
			consensus.logger.Infof("xqh [%s](%d/%d/%s) addVote %v with hasTwoThirdsAny!!!",
				consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep, vote)
			consensus.enterModifyPrecommit(consensus.ModifyBlockHeight,consensus.ModifyBlockRound)
		} else if consensus.ModifyBlockRound == vote.Round && voteSet.hasTwoThirdsNoMajority() {
			// add the prevote timeout event
			consensus.logger.Infof("xqh [%s](%d/%d/%s) addVote %v with hasTwoThirdsAny, PrevoteTimeout is igniting !!!",
				consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,vote)
			consensus.AddTimeout(consensus.PrevoteTimeout(consensus.ModifyBlockRound), consensus.Height,
				consensus.Round, tbftpb.Step_PRECOMMIT)
		}
		return
	}
	...
	//	(vote.Round <= consensus.Round) &&
	//	!bytes.Equal(hash, consensus.LockedProposal.Block.Header.BlockHash) {
	//	consensus.logger.Infof("xqh unlocking because of a higher round maj32 ,locked_round: %d, height_round: %d",
	//		consensus.LockedRound, vote.Round)
	//	// there is the unlocked action ,it means that we are ready to receive new proposal and vote for it
	//	consensus.LockedRound = -1
	//	consensus.LockedProposal = nil
	...
	//
	// Upon >2/3 prevotes, Step into StepPrecommit
	if consensus.ModifyProposal != nil {
		// recent non-nil block of maj32 .
		//这里不需要(consensus.ValidModifyRound < vote.Round)，修改可能是乱序的
		if !isNilHash(hash)  && (vote.Round == consensus.ModifyBlockRound) {
			if bytes.Equal(hash, consensus.ModifyProposal.PbMsg.Block.Header.BlockHash) {
				consensus.logger.Infof("xqh updating valid block because of maj32, valid_round: %d, majority_round: %d",
					consensus.ValidModifyRound, vote.Round)
				consensus.ValidModifyRound = vote.Round
				consensus.ValidModifyProposal = consensus.Proposal.PbMsg
				// ValidProposal need qc
				// The backup node receives a proposal containing qc, execute "VerifyBlockWithRwSets"
				hashStr := base64.StdEncoding.EncodeToString(hash)
				for _, v := range voteSet.VotesByBlock[hashStr].Votes {
				...
				consensus.logger.Infof("xqh modify proposal [%x] is not match majority[%x], need new round, set ProposalBlock=nil",
					consensus.ModifyProposal.PbMsg.Block.Header.BlockHash, hash)
				// we need a new round
				consensus.ModifyProposal = nil
			}
		}
		consensus.logger.Infof("xqh 准备进入enterModifyPrecommit")
		consensus.enterModifyPrecommit(consensus.ModifyBlockHeight, consensus.ModifyBlockRound)
	} else {
		if isNilHash(hash) {
			consensus.logger.Infof("xqh 准备进入enterModifyPrecommit")
			...
			consensus.logger.Infof("xqh [%s](%d/%d/%s) add vote failed, receive valid block: %x, but proposal is nil!!!",
				consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep, hash)
			consensus.logger.Infof("xqh 准备进入enterModifyPrecommit")
			consensus.enterModifyPrecommit(consensus.ModifyBlockHeight, consensus.ModifyBlockRound)
		}
	}
...
func (consensus *ConsensusTBFTImpl) addModifyPrecommitVote(vote *tbftpb.Vote) {
	if consensus.ModifyConsensusStep != tbftpb.Step_PRECOMMIT {
		consensus.logger.Infof("xqh [%s](%d/%d/%s) add modify precommit vote %s(%d/%d/%x) at inappropriate step",
			consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,
			vote.Voter, vote.Height, vote.Round, vote.Hash)
		return
	}

	voteSet := consensus.modifyHeightRoundVoteSet.precommits(vote.Round)
	hash, ok := voteSet.twoThirdsMajority()
	if !ok || voteSet.Round != consensus.ModifyBlockRound {
		consensus.logger.Infof("xqh [%s](%d/%d/%s) add Modify Precommit Vote %v without majority",
			consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep, vote)

		if consensus.ModifyBlockRound == vote.Round && voteSet.hasTwoThirdsAny() {
			consensus.logger.Infof("[%s](%d/%d/%s) add Modify Precommit Vote %v with hasTwoThirdsAny",
				consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep, vote)
			consensus.logger.Infof("xqh 准备进入 enterModifyCommit")
			consensus.enterModifyCommit(consensus.ModifyBlockHeight, consensus.ModifyBlockRound)
		} else if consensus.ModifyBlockRound == vote.Round && voteSet.hasTwoThirdsNoMajority() {
			// add the precommit timeout event
			consensus.logger.Infof("[%s](%d/%d/%s) add Modify Precommit Vote %v with hasTwoThirdsNoMajority, PrecommitTimeout is igniting",
				consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep, vote)
			consensus.AddTimeout(consensus.PrecommitTimeout(consensus.ModifyBlockRound), consensus.ModifyBlockHeight,
				consensus.ModifyBlockRound, tbftpb.Step_COMMIT)
		}
		return
	}

	consensus.logger.Infof("xqh [%s](%d/%d/%s) modify precommitQC (%d/%d/%x)",
		consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep, vote.Height, vote.Round, hash)

	// Upon >2/3 precommits, Step into StepCommit
	if consensus.ModifyProposal != nil {
		if isNilHash(hash) || bytes.Equal(hash, consensus.ModifyProposal.PbMsg.Block.Header.BlockHash) {
			consensus.logger.Infof("xqh 准备进入 enterModifyCommit")
			consensus.enterModifyCommit(consensus.ModifyBlockHeight, consensus.ModifyBlockRound)
		} else {
			consensus.logger.Infof("[%s](%d/%d/%s) block matched failed, receive valid block: %x,"+
				" but unmatched with proposal: %x",
				consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,
				hash, consensus.ModifyProposal.PbMsg.Block.Header.BlockHash)
		}
	} else {
		if !isNilHash(hash) {
			consensus.logger.Warnf("[%s](%d/%d/%s) receive valid block: %x, but proposal is nil",
...
// enterPrecommit enter `precommit` phase
func (consensus *ConsensusTBFTImpl) enterModifyPrecommit(height uint64, round int32) {
	consensus.logger.Infof("xqh 成功进入enterModifyPrecommit [%s](%v/%v/%v) ",consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,)
	if consensus.ModifyBlockHeight != height ||
		(consensus.ModifyBlockRound == round && consensus.ModifyConsensusStep != tbftpb.Step_PREVOTE) {
		consensus.logger.Infof("xqh [%s](%v/%v/%v) enter modify precommit invalid(%v/%v)",
			consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep, height, round)
		return
	}
	...
	// Enter StepPrecommit
	consensus.ModifyConsensusStep = tbftpb.Step_PRECOMMIT
	//consensus.metrics.SetEnterPrecommitTime(consensus.Round)
	//
	voteSet := consensus.modifyHeightRoundVoteSet.prevotes(consensus.ModifyBlockRound)
	//hash, ok := voteSet.twoThirdsMajority()
	hash, _ := voteSet.twoThirdsMajority()
	// del invalid txs
	//consensus.delInvalidTxs(voteSet, hash)
	//if !ok {
	//	if voteSet.hasTwoThirdsAny() || voteSet.hasTwoThirdsNoMajority() {
	//		hash = nilHash
	//		consensus.logger.Infof("xqh [%s](%v/%v/%v) enter modify precommit to nil because hasTwoThirdsAny "+
	//			"or hasTwoThirdsNoMajority", consensus.Id, consensus.Height, consensus.Round, consensus.Step)
	//	} else {
	//		consensus.logger.Errorf("panic: this should not happen")
	//		panic("this should not happen")
	...
	//	// There was a maj32 in the prevote set
	//	switch {
	//	case isNilHash(hash)://是空哈希
	//		//则根据当前的锁定状态来解锁或维持现状。
	//		// +2/3 prevoted nil. Unlock and precommit nil.
	//		if consensus.LockedModifyProposal == nil {
	//			consensus.logger.Infof("xqh precommit step; +2/3 prevoted for nil")
	//		} else {
	//			consensus.logger.Infof("xqh precommit step; +2/3 prevoted for nil; unlocking")
	//			consensus.LockedModifyRound = -1
	//			consensus.LockedModifyProposal = nil
	//		}
	//		//如果当前节点已经锁定了一个提案，且这个提案的区块哈希与获得多数支持的哈希相同，继续锁定该区块并更新锁定轮次。
	//	case consensus.LockedModifyProposal != nil && bytes.Equal(hash, consensus.LockedModifyProposal.Block.Header.BlockHash):
	//		// If we're already locked on that block, precommit it, and update the LockedRound
	//		consensus.logger.Infof("xqh precommit step; +2/3 prevoted locked block; relocking")
	//		consensus.LockedModifyRound = round
	//		//如果当前轮次的提案区块哈希获得了多数支持，锁定该提案并更新状态。
	//	case consensus.Proposal != nil && bytes.Equal(hash, consensus.Proposal.PbMsg.Block.Header.BlockHash):
	//		// If +2/3 prevoted for proposal block, locked and precommit it
	//		consensus.logger.Infof("xqh precommit step; +2/3 prevoted proposal block; locking ,hash = %x", hash)
	//		consensus.LockedModifyRound = round
	//		consensus.LockedModifyProposal = consensus.Proposal.PbMsg
	//		//默认情况：如果获得多数支持的区块不是当前已锁定或提议的区块，视为对未知区块的支持，因此投票“空”并解除任何现有的锁定。
	//	default:
	//		// There was a maj32 in this round for a block we don't have
	//		consensus.logger.Infof("xqh precommit step; +2/3 prevotes for a block we do not have; voting nil")
	//		consensus.LockedModifyRound = -1
	//		consensus.LockedModifyProposal = nil
	//		hash = nilHash
	//	}
	...
	// Broadcast precommit
	precommit := NewVote(tbftpb.VoteType_VOTE_MODIFYPRECOMMIT, consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, hash)
	err := consensus.signVote(precommit)
	if err != nil {
		consensus.logger.Errorf("enter Precommit sign Vote error: %s", err)
	}
	//signPrecommitTime := CurrentTime()
	//
	precommitMsg := createPrecommitModifyMsg(precommit)
	consensus.internalMsgC <- precommitMsg

	consensus.logger.Infof("xqh 生成precommit msg,发送给内部通道")
		//consensus.Id, consensus.Height, consensus.Round, consensus.Step,
		//precommit.Height, precommit.Round, precommit.Hash,
		//signPrecommitTime.Sub(enterPrecommitTime))
}
...
// enterCommit enter `Commit` phase
func (consensus *ConsensusTBFTImpl) enterModifyCommit(height uint64, round int32) {
	consensus.logger.Infof("xqh 成功进入enterModifyCommit")
	if consensus.ModifyBlockHeight != height ||
		consensus.ModifyBlockRound > round ||
		(consensus.ModifyBlockRound == round && consensus.ModifyConsensusStep != tbftpb.Step_PRECOMMIT) {
		consensus.logger.Infof("xqh [%s](%d/%d/%s) enterCommit invalid(%v/%v)",
			consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep, height, round)
		return
	}

	consensus.logger.Infof("xqh [%s](%d/%d/%s) enter commit (%v/%v)",
		consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep, height, round)
	consensus.logger.Infof("xqh 成功进入Step_COMMIT阶段")
	// Enter StepCommit
	consensus.ModifyConsensusStep = tbftpb.Step_COMMIT
	//consensus.metrics.SetEnterCommitTime(consensus.Round)
	//consensus.logger.Infof("[%s] consensus cost: %s", consensus.Id, consensus.metrics.roundString(consensus.Round))

	voteSet := consensus.modifyHeightRoundVoteSet.precommits(consensus.Round)
	//hash, ok := voteSet.twoThirdsMajority()
	//if !isNilHash(hash) && !ok {
	//	This should not happen
		//consensus.logger.Errorf("xqh [%s]-%x, enter modify commit failed, without majority", consensus.Id, hash)
		//panic(fmt.Errorf("xqh[%s]-%x, enter modify commit failed, without majority", consensus.Id, hash))
	//}

	//if consensus.LockedModifyProposal != nil && bytes.Equal(hash, consensus.LockedModifyProposal.Block.Hash()) {
	//	if consensus.ModifyProposal == nil ||
	//		(consensus.ModifyProposal != nil && !bytes.Equal(consensus.ModifyProposal.PbMsg.Block.Hash(),
	//			consensus.LockedModifyProposal.Block.Hash())) {
	//		consensus.logger.Infof ("xqh commit is for a locked block; set ProposalBlock=LockedBlock, hash = %x", hash)
	//		if consensus.isProposer(consensus.Height, consensus.Round) {
	//			consensus.ModifyProposal = NewTBFTProposal(consensus.LockedModifyProposal, true)
	//		} else {
	//			consensus.ModifyProposal = NewTBFTProposal(consensus.LockedModifyProposal, false)
	//		}
	//	}
	//
	//}
	//consensus.logger.Infof("xqh precommits 投票验证成功")
	//if consensus.ModifyProposal != nil && !bytes.Equal(hash, consensus.ModifyProposal.PbMsg.Block.Hash()) {
	//	consensus.logger.Infof("xqh 将 hash = nilHash")
	//	hash = nilHash
	//}
	//if consensus.ModifyProposal != nil{
	//	consensus.logger.Infof("consensus.ModifyProposal != nil")
	//}
	//if !bytes.Equal(hash, consensus.ModifyProposal.PbMsg.Block.Hash()) {
	//	consensus.logger.Infof("xqh hash和 consensus.ModifyProposal.PbMsg.Block.Hash()不相等，hash: %x with proposal hash: %x",hash, consensus.ModifyProposal.PbMsg.Block.Header.BlockHash)
	//}
	//if isNilHash(hash) || consensus.ModifyProposal == nil {
	//	consensus.AddTimeout(consensus.CommitTimeout(round), consensus.Height, round+1, tbftpb.Step_NEW_ROUND)
		//consensus.enterNewRound(consensus.Height, round+1)
	//} else {
		// Proposal block hash must be match with precommited block hash
		//if !bytes.Equal(hash, consensus.ModifyProposal.PbMsg.Block.Header.BlockHash) {
			// This should not happen
			//consensus.logger.Errorf("xqh [%s] block match failed, unmatch precommit hash: %x with proposal hash: %x",
			//	consensus.Id, hash, consensus.ModifyProposal.PbMsg.Block.Header.BlockHash)
			//panic(fmt.Errorf("xqh [%s] block match failed, unmatch precommit hash: %x with proposal hash: %x",
			//	consensus.Id, hash, consensus.ModifyProposal.PbMsg.Block.Header.BlockHash))
		//}
		consensus.logger.Infof("xqh 准备调用commitModifyBlock")
		// Commit block to core engine
		consensus.commitModifyBlock(consensus.ModifyProposal.PbMsg.Block, voteSet.ToProto())
	//}
}

func (consensus *ConsensusTBFTImpl) commitModifyBlock(block *common.Block, voteSet *tbftpb.VoteSet) {
	consensus.logger.Infof("xqh [%s] commitBlock to %d-%x",
		consensus.Id, block.Header.BlockHeight, block.Header.BlockHash)
	beginTime := CurrentTime()
	//qc := mustMarshal(voteSet)
	marshalTime := CurrentTime()
	...
	//block.AdditionalData.ExtraData[TBFTAddtionalDataKey] = qc
	consensus.logger.Infof("xqh 准备publish msgbus.RecvModifyBlock")
	consensus.msgbus.Publish(msgbus.RecvModifyBlock, block)

	consensus.logger.Infof("xqh [%s](%d/%d/%s) 对修改请求共识完毕，准备在本地修改区块 (%d/%x), time[marshalQC:%dms]",
		consensus.Id, consensus.ModifyBlockHeight, consensus.ModifyBlockRound, consensus.ModifyConsensusStep,
		block.Header.BlockHeight, block.Header.BlockHash,
		marshalTime.Sub(beginTime).Milliseconds())
}